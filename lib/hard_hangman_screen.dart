...
class HardHangmanScreen extends StatefulWidget {
  const HardHangmanScreen(
  ...
  @override
  State<HardHangmanScreen> createState() => _HardHangmanScreenState();
}

class _HardHangmanScreenState extends State<HardHangmanScreen> {
  //Firebase
      ...
      body: Column(
        ...
        children: <Widget>[
          ...
          if (hangmanCount == 7)
            RawMaterialButton(
              onPressed: () async {
                int highscoreHard = await getHighscore();
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(
                    builder: (context) => ScoreShowcasePage(
                        score: scoreHard, highScore: highscoreHard),
                  ),
                );
              },
              child: const Text(
  ...
  Future<int> getHighscore() async {
    ...
    final int highScoreHard = doc['Hard High Score'];
    if (highScoreHard < scoreHard) {
      final docId = query.docs.first.id;
      await firebaseFirestore
          .collection('users')
          .doc(docId)
          .update({'Hard High Score': scoreHard});
      return scoreHard;
    }
